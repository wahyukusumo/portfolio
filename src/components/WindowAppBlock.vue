<template>
  <div class="bg-gray-400/60 dark:bg-gray-900/60 w-screen h-screen top-0 left-0 fixed
                  lg:rounded-xl lg:z-30 lg:p-20 lg:flex lg:justify-center lg:items-center">
    <div class="lg:drop-shadow-lg animate-zoom-out"
      :class="{'lg:w-4/6 2xl:w-3/5': !isFull, 'w-full h-full top-7 absolute': isFull}">
      <!-- App Navbar -->
      <div class="bg-white relative w-full border-b border-gray-300
          dark:border-black p-2 grid grid-cols-3 text-xl dark:bg-[#231d26] dark:text-white"
      ...
      <!-- Content -->
      <div class="relative2 text-black bg-white h-screen lg:h-full lg:min-h-[40vh] overflow-y-scroll
          lg:pb-0 dark:bg-[#231d26] dark:text-white" style="scrollbar-width: thin;"
        :class="{'lg:max-h-[60vh]': !isFull, 'lg:max-h-screen': isFull}">
        <slot></slot>
      </div>
