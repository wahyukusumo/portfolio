<template>
  <div class="flex flex-col justify-center items-center">
    <!-- TOOLTIP -->
    <div ref="tooltipRef" :class="{ 'xl:hidden': !tooltipShow, 'xl:block': tooltipShow }"
    ...
    <!-- ICON -->
    <div @click="openingApp" :class="[bgColor, bgImage, { 'lg:-translate-y-1': openApp }]" ref="btnRef"
      @mouseenter="toggleTooltip()" @mouseleave="toggleTooltip()"
      class="w-14 h-14 mx-auto p-2 rounded-[13.3px] cursor-pointer bg-no-repeat bg-cover xl:hover:-translate-y-6 xl:transition-all xl:hover:scale-[3] xl:hover:mx-5" />
  ...
  <WindowApp v-show="openApp" :openApp="openApp" :appName="appName" @closeApp="openingApp"
    @appID="(id) => this.$emit('appID', id)" @windowFull="checkFull">
    <slot></slot>
  </WindowApp>
</template>
