...
<template>
  <Icon
    :appName="'Photos'"
    :bgImage="'bg-[url(@/assets/icons/camera_alt.webp)]'"
    @windowFull="(isParentFull) => (isFull = isParentFull)"
  >
    <!-- Full Image View -->
    <div v-show="imgFullScreen" class="flex items-center justify-center w-full h-full">
      <button
        class="absolute z-30 p-3 m-4 top-0 right-0 rounded-full bg-white/40"
        @click="imgFullScreen = false"
      >
        <XMarkIcon class="size-6 stroke-gray-600 hover:stroke-white" />
      </button>
      <img
        :src="imgFullScreen"
        class="absolute z-20 shadow-md overflow-x-auto"
        :class="{ 'h-[90vh]': isFull, 'h-[60vh]': !isFull }"
      />
      <div
        class="w-full h-[60vh] bg-gray-400 dark:bg-gray-700 blur-xl cursor-pointer"
        @click="imgFullScreen = false"
      />
    </div>
    ...
    <div
      ...
      class="p-2 gap-2 grid grid-cols-1 md:grid-cols-3 xl:p-4 relative"
      :class="{ 'xl:grid-cols-5': isFull }"
    >
      <div class="container mx-auto relative cursor-pointer" v-for="image in appdata.galleries">
        <div
          class="h-80 xl:h-56 relative rounded-lg"
          :class="{ 'xl:h-[22rem]': isFull }"
          @click="imgFullScreen = image.image"
        >
          <!-- Image -->
          <div
            class="absolute inset-0 bg-cover bg-center z-0 rounded-lg"
